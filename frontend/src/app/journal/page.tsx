...
import { JournalEntryForm, type ExistingEntry } from "@/components/journal/entry-form";
...
type FormTarget = { cycle?: CycleRow; entry?: ExistingEntry };
...
export default function JournalPage() {
  ...
  const entries = useQuery({
    ...
    queryFn: () => apiGet<{ entries: ExistingEntry[] }>("/journal/entries"),
  });
  const closeForm = () => {
    setForm(null);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setForm({})}
            className="press rounded-sm bg-accent px-3 py-1 text-[12px] font-medium text-accent-fg transition-opacity hover:opacity-90"
          >
            + רשומה חדשה
          </button>
          <button
      ...
      {form && (
        <JournalEntryForm cycle={form.cycle ?? null} entry={form.entry ?? null} onClose={closeForm} />
      )}

      <Panel
        title="רשומות יומן"
        subtitle="Journal Entries"
        actions={<span className="tag">{entries.data?.entries.length ?? 0}</span>}
      >
        {entries.isLoading ? (
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="t-label border-b border-line">
                  <th className="pb-1.5 text-start font-normal">תאריך</th>
                  <th className="pb-1.5 text-start font-normal">אסטרטגיה</th>
                  <th className="pb-1.5 text-start font-normal">מצב רגשי</th>
                  <th className="pb-1.5 text-start font-normal">דירוג</th>
                  <th className="pb-1.5 text-start font-normal">לקח מרכזי</th>
                  <th className="pb-1.5 text-end font-normal"></th>
                </tr>
              </thead>
              <tbody>
                {entries.data.entries.map((e) => (
                  <tr key={e.id} className="border-t border-line hover:bg-hover">
                    <td className="num py-1.5 text-[11.5px] text-muted">
                      {e.entry_date ? new Date(e.entry_date).toLocaleDateString("he-IL") : "—"}
                    </td>
                    <td className="py-1.5">{e.strategy ?? <span className="text-faint">—</span>}</td>
                    <td className="py-1.5 text-[11.5px] text-muted">{e.emotional_state ?? "—"}</td>
                    <td className="num py-1.5">{e.rating ? "★".repeat(e.rating) : "—"}</td>
                    <td className="max-w-xs truncate py-1.5 text-[11.5px] text-muted" title={e.key_lesson ?? ""}>
                      {e.key_lesson ?? "—"}
                    </td>
                    <td className="py-1.5 text-end">
                      <button
                        type="button"
                        onClick={() => setForm({ entry: e })}
                        className="rounded-sm border border-line px-2 py-0.5 text-[10.5px] text-muted hover:bg-subtle hover:text-fg"
                      >
                        ערוך
                      </button>
          ...
          <div className="py-8 text-center">
            <p className="text-[13px] font-medium">אין עדיין רשומות יומן</p>
            <p className="mx-auto mt-1 max-w-md text-[12px] text-muted">
              לחץ על <span className="font-medium text-fg">+ רשומה חדשה</span> כדי לכתוב רשומה עצמאית, או
              הוסף רשומה למחזור עסקה מהטבלה למעלה.
            </p>
          </div>
        )}
      </Panel>
